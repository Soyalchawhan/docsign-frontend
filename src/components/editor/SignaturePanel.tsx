  ...
  return (
    <div className="card p-4">
      <h3 className="font-medium text-white text-sm mb-4 flex items-center gap-2">
        ...
        Signatures ({allSigs.length})
      </h3>

      {allSigs.length === 0 ? (
        <div className="text-center py-6">
          <p className="text-slate-500 text-xs">No signature fields yet.</p>
          <p className="text-slate-600 text-xs mt-1">Click "Add Signature Field" to place one.</p>
        </div>
      ) : (
          ...
          {allSigs.map(sig => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-slate-300 font-medium truncate">{sig.signerName}</p>
                <p className="text-slate-500 truncate">{sig.signerEmail}</p>
                <p className="text-slate-600">Page {sig.page} · {sig.status}</p>
              </div>
              {sig.status === 'placed' && (
                <button

      ...
      <div className="border-b border-slate-800 bg-slate-900/50 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {allSigned && (
              <button onClick={handleFinalize} className="btn-primary text-sm">Generate Signed PDF</button>
            )}
            {doc.signedFilePath && (
              <button onClick={() => handleDownload(true)} className="btn-secondary text-sm">Download Signed</button>
            )}
            ...
            {signatures.filter(s => s.page === currentPage).map(sig => (
              <div
                ...
                style={{
                  ...
                  height: `${sig.height}%`,
                  position: 'absolute',
                  cursor: sig.status === 'placed' ? 'move' : 'default'
                }}

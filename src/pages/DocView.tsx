...
const DocView: React.FC = () => {
  ...
  const handlePdfClick = async (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    try {
      const res = await api.post('/api/signatures', {
        ...
        x, y, width: 22, height: 10, page: currentPage
      })
      setSignatures(prev => [...prev, res.data.signature])
      toast.success(`Field placed for ${selectedSigner.name}`)
      setPlacing(false)
    } catch (err: any) {
  ...
  const handleDragStart = (e: React.MouseEvent<HTMLDivElement>, sig: Signature) => {
    ...
    const onMove = (me: MouseEvent) => {
      ...
      setSignatures(prev => prev.map(s => s._id === sig._id
        ? { ...s, x: Math.max(0, Math.min(80, startLeft + dx)), y: Math.max(0, Math.min(90, startTop + dy)) }
        : s
      ))
    }
    ...
    <div className="min-h-screen bg-slate-950">
      <div className="border-b border-slate-800 bg-slate-900/50 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {!placing ? (
              <button onClick={() => setPlacing(true)} className="btn-primary flex items-center gap-2 text-sm">+ Add Signature Field</button>
            ) : (
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 flex gap-6">
        <div className="flex-1 min-w-0 flex flex-col items-center">
          {placing && (
            <div className="card p-4 mb-4 w-full max-w-3xl">
              <p className="text-sm text-slate-400 mb-3">Select signer then click on the PDF:</p>
              <div className="flex flex-wrap gap-2">
                {doc.signers?.map((signer, i) => (
                  <button key={i} onClick={() => setSelectedSigner(signer)}
                    className={`px-3 py-2 rounded-lg text-sm transition-colors ${selectedSigner?.email === signer.email ? 'bg-ink-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>
                    {signer.name}{signer.role && <span className="text-xs opacity-70 ml-1">· {signer.role}</span>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              {doc.signers?.map((signer, i) => (
                <div key={i} className="flex items-center gap-2 text-xs">
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${signer.status === 'signed' ? 'bg-emerald-400' : signer.status === 'rejected' ? 'bg-red-400' : 'bg-amber-400'}`} />
                  ...
                  <span className={`flex-shrink-0 ${signer.status === 'signed' ? 'text-emerald-400' : signer.status === 'rejected' ? 'text-red-400' : 'text-amber-400'}`}>{signer.status}</span>
                </div>
              ))}
            </div>
          </div>

          <SignaturePanel signatures={signatures} currentPage={currentPage} onDelete={handleDeleteSignature} />
